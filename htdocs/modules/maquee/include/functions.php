<?php
if (!function_exists('maquee_getFilterElement')) {
	function maquee_getFilterElement($filter, $field, $sort='created', $op = '', $fct = '') {
		$components = maquee_getFilterURLComponents($filter, $field, $sort);
		$ele = false;
		include_once('formobjects.maquee.php');
		switch ($field) {
			case 'type':
				$ele = new MaqueeFormSelectType('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	...
		    	break;
			case 'language':
				$ele = new MaqueeFormSelectLanguage('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;
			case 'animation':
				$ele = new MaqueeFormSelectAnimation('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;
			case 'sequence':
				$ele = new MaqueeFormSelectSequence('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;
			case 'mid':
				switch($op) {
				default:
					$ele = new MaqueeFormSelectMaquee('', 'filter_'.$field.'', $components['value'], 1, false, true);
					break;
				case 'gallery': 
					$ele = new MaqueeFormSelectMaquee('', 'filter_'.$field.'', $components['value'], 1, false, true, '_MI_MAQUEE_TYPE_GALLERY');
					break;
				case 'text':
					$ele = new MaqueeFormSelectMaquee('', 'filter_'.$field.'', $components['value'], 1, false, true, '_MI_MAQUEE_TYPE_TICKER');
					break;
				}
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;
...
if (!function_exists('maquee_getFilterURLComponents')) {
	function maquee_getFilterURLComponents($filter, $field, $sort='created') {
    	...
    	$pagenav = array();
    	$pagenav['op'] = isset($_REQUEST['op'])?$_REQUEST['op']:"maquee";
...
if (!function_exists("maquee_adminMenu")) {
  function maquee_adminMenu ($currentoption = 0)  {
		$module_handler = xoops_gethandler('module');
		$xoModule = $module_handler->getByDirname('maquee');
  ...
  function maquee_footer_adminMenu()
  {
		echo "</div></td></tr>";
  		echo "</table>";
  }
